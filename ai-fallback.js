...
async function aiFallback(text, phone) {
  ...
  try {
    var ctx = ""; try { var { redis } = require("./db"); var d = await redis.get("session:"+phone); if(d) { var s = JSON.parse(d); if(s.history) { ctx = s.history.slice(-6).map(function(m){ return (m.role==="user"?"Customer: ":"Bot: ")+m.content; }).join("\n"); } } } catch(e) {}
