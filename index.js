...
// ============================================================
// WHATSAPP CLIENT WITH @sparticuz/chromium - WORKS ON RENDER
// ============================================================
(async () => {
  ...
  client.on('message', async (message) => {
    try {
      ...
      if (isImage) {
        try {
          await sendMessage(from, ' Thanks! Your photo has been forwarded to Ridhor (076 760 4350).');
          await client.sendMessage(PERSONAL_NUMBER + '@c.us', ` Image from ${from}`);
          const media = await message.downloadMedia();
          if (media) await client.sendMessage(PERSONAL_NUMBER + '@c.us', media);
          return;
        } catch (e) {
          console.error('Image error:', e.message);
          await sendMessage(from, 'Could not process image.');
          return;
        }
      }
      if (!text) return;
      ...
      if (isAfterHours()) {
        ...
        if (showClosed) reply = reply + '\n\nOur workshop is closed (Mon-Thurs 8AM-4:45PM, Fri 8AM-2:45PM).';
      }
      ...
      if (/complaint|problem|unhappy|angry|furious|refund/.test(text)) {
        try { await client.sendMessage(PERSONAL_NUMBER + '@c.us', ` Complaint from ${from}: ${text}`); } catch(e) {}
      } else if (/speak.*ridhor|talk.*ridhor|technical|owner|boss/.test(text)) {
        try { await client.sendMessage(PERSONAL_NUMBER + '@c.us', ` Customer ${from} wants to talk: ${text}`); } catch(e) {}
      }
    } catch (error) {
  ...
  app.get("/health", function(req, res) { res.json({ status: "healthy", version: "17.0" }); });
  app.get("/", function(req, res) { res.json({ service: "Solomon Coatings", version: "17.0" }); });
  ...
  app.post("/webhook", validateWhatsAppSignature, async function(req, res) { res.sendStatus(200); });
