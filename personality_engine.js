// ============================================
// SOLOMON COATINGS - PERSONALITY ENGINE v3.0
// "The Workshop Lad With Opinions" + Roast + Voice
// ============================================
...
const PERSONA = {
  ...
  contractor_mode: {
    on: "Contractor mode ON. Fast quotes, 8% auto-discount. Let's work.",
    off: "Contractor mode OFF. Back to friendly Oom mode. Howzit!"
  },
  ...
  rust_roast: [
    "Ag shame, what did this gate do to deserve this? Don't worry — R8-R12/kg blasting and she'll look brand new. Weight?",
    "Yoh! That rust has been through things. Looks like it survived the apocalypse. We'll bring it back to life — weight and colour?",
    "This gate has seen better days. But that's why we exist. Blasting + coating = new gate vibes. How heavy is it?",
    "Eish, that's not rust, that's character. But we can erase it. Blasting R8-R12/kg. What colour you thinking after we rescue it?",
    "My oog! That's proper rusted. Don't stress — we've seen worse since '88. Weight estimate?"
  ],
  ...
  voice_note: [
    "Got your voice note! I can't listen to it directly, but Ridhor can. Forwarding it to him now. In the meantime — what's the job? Gate, rims, or something else?",
    "Voice note received! Ridhor will listen and get back to you. But if you want a quick quote, just type the weight and colour — I'm fast.",
    "Heard you sent a voice note! I'm forwarding it to the boss. For instant answers, text me: product + weight + colour. I'll quote in seconds."
  ],
  
  loadshedding: {
    stage0: "",
    stage1: "",
    stage2: "\n\nEskom Stage 2 today — no major delays expected.",
    stage3: "\n\nEskom Stage 3 — may add half a day to turnaround.",
    stage4: "\n\nEskom Stage 4 today — add 1 extra day to turnaround. We'll keep you posted.",
    stage5: "\n\nEskom Stage 5 — add 1-2 days. We run generator when we can.",
    stage6: "\n\nEskom Stage 6 — add 2-3 days. Major delays possible. We'll update you."
  },
  
  friday_vibe: [
    "It's Friday! Workshop closes at 2:45. Get your quote in quick!",
    "Friday! We knock off early (2:45). Still time for a quote though.",
    "End of the week! Workshop open till 2:45 today. What's the last job?"
  ]
};
...
var LOADSHEDDING_STAGE = 0;
var CONTRACTOR_MODE = false;
...
function detectProduct(text) {
  ...
  if (t.indexOf('gate') !== -1 || t.indexOf('fence') !== -1 || t.indexOf('burglar') !== -1) return 'gate';
  if (t.indexOf('rim') !== -1 || t.indexOf('wheel') !== -1 || t.indexOf('mag') !== -1) return 'rims';
  if (t.indexOf('chassis') !== -1 || t.indexOf('trailer') !== -1) return 'chassis';
  if (t.indexOf('sheet') !== -1 || t.indexOf('mesh') !== -1 || t.indexOf('panel') !== -1) return 'sheet_metal';
  if (t.indexOf('truck') !== -1 || t.indexOf('bakkie') !== -1 || t.indexOf('flatbed') !== -1) return 'truck';
  if (t.indexOf('bracket') !== -1 || t.indexOf('small') !== -1 || t.indexOf('part') !== -1) return 'small_parts';
  return 'unknown';
}

function detectCondition(text) {
  var t = text.toLowerCase();
  if (/heavy|bad|proper|severe|pitted|flaking|terrible|awful|horrible|destroyed/.test(t)) return 'heavy';
  if (/light|surface|bit|little|minor|small/.test(t)) return 'mild';
  if (/clean|new|fresh|good|bare/.test(t)) return 'clean';
  return 'unknown';
}

function detectMood(text) {
  var t = text.toLowerCase();
  if (/angry|furious|ridiculous|terrible|useless|rubbish|worst|pathetic|refund|lawyer|kak|swak|gemors/.test(t)) return 'angry';
  if (/tired|exhausted|late|midnight|cant sleep/.test(t)) return 'tired';
  if (/lol|haha|funny|joke/.test(t)) return 'joking';
  return 'neutral';
}
...
function isAppreciation(text) {
  var t = text.toLowerCase();
  return /thanks|thank you|dankie|shot|awesome|great|perfect|legend|amazing|brilliant|love it/.test(t);
}
...
function isAIQuestion(text) {
  var t = text.toLowerCase();
  return /(are you|you a).*(ai|bot|robot|real|human|person)/.test(t);
}
...
function getLoadsheddingNotice() {
  if (LOADSHEDDING_STAGE >= 2 && PERSONA.loadshedding["stage" + LOADSHEDDING_STAGE]) {
    return PERSONA.loadshedding["stage" + LOADSHEDDING_STAGE];
  }
  return "";
}

module.exports = {
  PERSONA, FLOW, NEXT_QUESTIONS, LOADSHEDDING_STAGE, CONTRACTOR_MODE,
  pick, getTimeOfDay, isFriday, getPersona,
  detectProduct, detectCondition, detectMood,
  isGreeting, isAppreciation, isGoodbye, isAIQuestion,
  detectAfrikaans, isReturningCustomer, getLoadsheddingNotice
};
